...
**Prerequisites:** Module 3 complete — `finovra` is `Synced`/`Healthy` at v1.0.0, with `automated`, `prune`, and `selfHeal` all enabled from Steps 6-8
...
The feature: **Finovra v1.0.0 → v2.0.0**, adding `insurance-service`. The Insurance tile — greyed out and "Coming Soon" since Module 3 — goes live.

---

## What's In This Release

Finovra's versioning is **lockstep** (see the root `README.md`): every release bumps *all* services to the same version number, even ones that didn't change, so the version number always describes "the whole app," not one service. That means v2.0.0 touches three things in one commit:
...
`arsr319/finovra-dashboard:2.0.0` and `arsr319/finovra-accounts-service:2.0.0` already exist on Docker Hub — byte-identical to their `:1.0.0` images, just retagged, exactly like the root README describes. You're not rebuilding anything this module; that's what Module 6 (CI) is for.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: insurance-service
          image: arsr319/finovra-insurance-service:2.0.0
          ports:
            - containerPort: 8000
          env:
            - name: PORT
...
Notice this is a **new folder**, not an edit to an existing file — and the Application spec you wrote in Module 3 already has `directory.recurse: true`. You don't need to touch `finovra-app.yaml` at all for ArgoCD to find it.
...
```yaml
image: arsr319/finovra-dashboard:1.0.0    # -> 2.0.0
...
- name: VERSION
  value: "1.0.0"                          # -> "2.0.0"
```
...
- `argocd app get finovra` settle back to `Sync Status: Synced`, `Health Status: Healthy`
...
In **Module 5**, we'll redeploy Finovra using its Helm chart instead of plain YAML, then compare that to a Kustomize-based deployment — two different ways to describe "what should be running" for the same app.
